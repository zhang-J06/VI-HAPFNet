...
def train(net, loader_train, loader_test, optim, criterion):
    ...
    for step in range(start_step + 1, steps + 1):
        ...
        loss = loss + 0.03 * loss_rgb_stru + 0.1 * (
                    1 - loss_mssim)  # adjust parameters weights according to reference,L_all
        ...
        print(f'\rloss:{loss.item():.5f} l2:{loss_rgb_stru:.5f}   '
              f'step :{step}/{steps}|lr :{lr :.7f} |time_used :{(time.time() - start_time) / 60 :.1f} min', end='',
              flush=True)
...
# 自定义 PReLU 的 FLOPs 统计（跳过，返回 None）
def prelu_hook(m, x, y):
    return
...
if __name__ == "__main__":
    ...
    # === Step 1: 构建模型用于统计 FLOPs ===
    net = vifnet(3, 3).to(opt.device)

    # Params
    pytorch_total_params = sum(p.numel() for p in net.parameters() if p.requires_grad)
    print("Total_params (M): ==> {:.2f}".format(pytorch_total_params / 1e6))

    # # FLOPs（假设输入 256×256，可根据需要修改）
    # dummy_rgb = torch.randn(1, 3, 256, 256).to(opt.device)
    # dummy_nir = torch.randn(1, 3, 256, 256).to(opt.device)
    #
    # custom_ops = {torch.nn.PReLU: prelu_hook}
    # flops, params = profile(net,
    #                         inputs=(dummy_rgb, dummy_nir),
    #                         custom_ops=custom_ops)  # 跳过 PReLU
    # flops, params = clever_format([flops, params], "%.3f")
    # print("FLOPs (G): ==> {}".format(flops))
    # print("Params (M): ==> {}".format(params))
    #
    # # === Step 2: 重新构建模型，避免 hook 干扰训练 ===
    # net = vifnet(3, 3).to(opt.device)
