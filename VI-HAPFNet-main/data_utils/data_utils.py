...
class RGB_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        clear = np.asarray(Image.fromarray(clear).resize((self.input_w, self.input_h)), dtype=np.float32).transpose(
            (2, 0, 1)) / 255
        # nir_name = id + '_thermal_foggy_1.0.png'
        nir_name = id
...
path = '/root/autodl-tmp/data/NH-HAZE'
# path = '/root/autodl-tmp/data/Dense-Haze' # path to your 'dataset' folder
# path = '/root/autodl-tmp/data/foggy_1.0'
RGB_train_loader = DataLoader(dataset=RGB_Dataset(path+'/train',
                               train=True, format='.png'), batch_size=BS, shuffle=True)
RGB_test_loader = DataLoader(dataset=RGB_Dataset(path+'/val',
